    ...
    <section aria-labelledby="sub-heading" className="py-20 sm:py-24 bg-white">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <span className="inline-block rounded-lg bg-purple-50 px-3 py-1 text-sm font-semibold text-purple-700 mb-5">
              For Subscription Users
            </span>
            <h2 id="sub-heading" className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">
            ...
            <ul className="space-y-3 mb-8" aria-label="Subscription visibility features">
              {points.map((p) => (
                <li key={p} className="flex items-start gap-3 text-gray-700">
                  <span aria-hidden="true" className="mt-1 shrink-0 text-blue-600">✓</span>
                  <span>{p}</span>
                </li>
              ))}
            </ul>
              ...
              {mockRows.map((row) => (
                <div key={row.label} className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <dd className="text-xs text-gray-400">{row.sub}</dd>
                  </div>
                  <span className="font-mono text-sm font-bold text-gray-900">{row.value}</span>
                </div>
              ))}
            </dl>
          </div>
